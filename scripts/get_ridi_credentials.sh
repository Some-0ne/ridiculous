...
open_login_page() {
    ...
    if [[ "$NO_BROWSER" == "true" ]]; then
        ...
        if command -v open &> /dev/null; then
            # macOS
            open "$login_url" && echo "Opening in default browser..."
        elif command -v xdg-open &> /dev/null; then
            # Linux
            xdg-open "$login_url" && echo "Opening in default browser..."
        elif command -v start &> /dev/null; then
            # Windows (Git Bash, WSL)
            start "$login_url" && echo "Opening in default browser..."
        else
            print_warning "Could not automatically open browser"
            echo "  Please manually open: $login_url"
        fi
...
get_user_profile() {
    ...
    if [[ "$NO_BROWSER" == "true" ]]; then
        ...
        echo "Opening this URL in a new tab (keep the login tab open):"
        echo
        echo -e "${BOLD}$profile_url${NC}"
...
get_device_info() {
    ...
    if [[ "$NO_BROWSER" == "true" ]]; then
        ...
        # Try to open the API URL
        if command -v open &> /dev/null; then
            open "$api_url" && echo "Opening API endpoint in browser..."
        elif command -v xdg-open &> /dev/null; then
            xdg-open "$api_url" && echo "Opening API endpoint in browser..."
        elif command -v start &> /dev/null; then
            start "$api_url" && echo "Opening API endpoint in browser..."
        else
            print_warning "Could not automatically open browser"
            echo "  Please manually open: $api_url"
        fi
...
ensure_executable() {
    ...
    if [[ ! -x "$script_path" ]]; then
        ...
        if chmod +x "$script_path" 2>/dev/null; then
            print_success "Script is now executable"
            echo "You can now run it directly with: $script_path"
        else
            print_warning "Could not make script executable automatically"
            echo "Please run one of these commands first:"
            echo "  chmod +x $script_path"
            echo "  Or run with: bash $script_path"
        fi
        echo
    fi
}
...
check_permissions() {
    # Check if we're running as the script itself or via bash/sh
    if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
        # Running directly - check if executable
        if [[ ! -x "${BASH_SOURCE[0]}" ]]; then
            print_error "Permission denied - script is not executable"
            echo
            echo "Fix this by running one of:"
            echo -e "  ${CYAN}chmod +x ${BASH_SOURCE[0]}${NC}"
            echo -e "  ${CYAN}bash ${BASH_SOURCE[0]}${NC}"
            echo
            exit 1
        fi
    fi
...
main() {
    ...
    # Try to make executable for future runs
    ensure_executable
...
show_help() {
    ...
    echo
    echo "If you get permission denied errors, try:"
    echo "  chmod +x $0"
    echo "  bash $0"
    echo
}
