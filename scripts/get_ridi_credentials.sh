...
match_system_to_devices() {
    ...
    local current_os="${system_info[0]}"
    local current_model="${system_info[1]}"
    local -a search_terms=("${system_info[@]:2}")  # Additional search terms
...
validate_device_id() {
    ...
    if [[ "$device_id" =~ ^[a-zA-Z0-9]{8}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{12}$ ]]; then
        return 0  # UUID-like format
    fi
    
    # Tertiary check: Alternative formats that might be used
    # 32-character hex string without dashes
    if [[ "$device_id" =~ ^[a-fA-F0-9]{32}$ ]]; then
        return 0  # Hex string format
    fi
    
    # Fallback: Any reasonable device ID pattern
    if [[ "$device_id" =~ ^[a-zA-Z0-9-]{20,50}$ ]] && [[ "$device_id" == *"-"* ]]; then
        return 0  # Generic device ID pattern
    fi
...
validate_user_idx() {
    ...
    if [[ "$user_idx" =~ ^[0-9]{6,15}$ ]]; then
        return 0  # Valid numeric user index
    fi
...
parse_manually() {
    ...
    local current_os="${system_info%% *}"  # First word is OS
...
# Run main script
main\t' read -r device_id user_idx device_name os device_type; do
            if [[ -n "$device_id" ]]; then
                local match_score=0
                local match_reasons=()
...
# Enhanced JSON parsing with better pattern matching and device ID recognition
parse_with_jq() {
    echo "Great! Let's parse the JSON to find your credentials."
    echo
    echo "Please copy the ENTIRE JSON response from the browser and paste it here."
    ...
    # Try to parse with jq - look for both device_id and user_idx patterns
    local devices
    if devices=$(echo "$json_content" | jq -r '.result[]? | select(.device_id and .user_idx) | "Device: \(.device_name // .device_type // "Unknown") [\(.os // "Unknown OS")] - ID: \(.device_id) - User Index: \(.user_idx)"' 2>/dev/null); then
        if [[ -n "$devices" ]]; then
            ...
            # Extract and validate device IDs with enhanced recognition
            ...
            if [[ -n "$device_ids" ]]; then
                ...
                while IFS= read -r did; do
                    if validate_device_id "$did"; then
                        local id_type=$(classify_device_id "$did")
                        echo -e "  ${count}. ${GREEN}$did${NC} ✅ Valid ($id_type)"
                    else
                        echo -e "  ${count}. ${YELLOW}$did${NC} ⚠️  Unusual format (length: ${#did})"
                    fi
                    ((count++))
                done <<< "$device_ids"
                echo
                
                echo -e "${CYAN}💡 Recommended choice:${NC} Look for UUID format (8-4-4-4-12 pattern)"
                echo "   This typically works best with RIDI decryption tools."
                echo
            fi
            ...
            if [[ -n "$user_indices" ]]; then
                ...
                while IFS= read -r uidx; do
                    if validate_user_idx "$uidx"; then
                        echo -e "  ${count}. ${GREEN}$uidx${NC} ✅ Valid format (${#uidx} digits)"
                    else
                        echo -e "  ${count}. ${YELLOW}$uidx${NC} ⚠️  Expected numbers only"
                    fi
                    ((count++))
                done <<< "$user_indices"
                echo
...
# Enhanced manual parsing with format validation and better guidance
parse_manually() {
    echo -e "${YELLOW}📝 Let's find your credentials manually:${NC}"
    echo
    echo "In the JSON you see in your browser, look for patterns like:"
    ...
    # Get device ID with enhanced validation and feedback
    while true; do
        echo
        read -p "Please enter your device_id: " input_device_id
...
main\t' read -r device_id user_idx device_name os device_type; do
                if [[ -n "$device_id" ]]; then
                    ...
                    if validate_device_id "$device_id"; then
                        local id_type=$(classify_device_id "$device_id")
                        echo -e "     Device ID: ${GREEN}$device_id${NC} ✅ ($id_type)"
                    else
                        echo -e "     Device ID: ${YELLOW}$device_id${NC} ⚠️  (length: ${#device_id})"
                    fi
                    ...
                    if validate_user_idx "$user_idx"; then
                        ...
                        echo -e "     User Index: ${YELLOW}$user_idx${NC} ⚠️  (${#user_idx} digits)"
                    fi
